...
export const createStripeCustomer = async (
    ...
    const createdcustome = await stripe.customers.create({
        email: input.email,
        name: input.name
    })

    const createdCustomeSubscription = await stripe.subscriptions.create({
        customer: createdcustome.id,
    ...
    await prisma.user.update({
        ...
        data:{
            stripeCustomerId: createdcustome.id,
            stripeSubscriptionId: createdCustomeSubscription.id,
            stripeSubscriptionStatus: createdCustomeSubscription.status,
            stripePriceId: config.stripe.plans.free.priceId
        }
    })
    return createdcustome
    }
